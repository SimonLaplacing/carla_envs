...
class PPO:
    ...
    def select_action(self, state1, state2):
        state1 = torch.FloatTensor(state1).to(device)
        state2 = torch.FloatTensor(np.array(state2)/255).to(device)
        if self.has_continuous_action_space:
            with torch.no_grad():
                
                action, action_logprob = self.policy_old.act(state1, state2)

            self.buffer.state1.append(state1)
            self.buffer.state2.append(state2)
    ...
    def update(self):
        ...
        old_state1 = torch.squeeze(torch.stack(self.buffer.state1, dim=0)).detach().to(device)
        old_state2 = torch.squeeze(torch.stack(self.buffer.state2, dim=0)).detach().to(device)
        ...
        for _ in range(self.K_epochs):
            ...
            logprobs, state_values, dist_entropy = self.policy.evaluate(old_state1, old_state2, old_actions)
