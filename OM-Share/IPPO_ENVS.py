...
class Create_Envs(object):
    ...
    def get_vehicle_step(self,ego,npc,ego_sensor,npc_sensor,ego_route,npc_route,extra_reward):
        ...
        ego_target_disY = (ego_route.location.y - ego_next_transform.location.y)/10
        ...
        ego_reward = (-5)*ego_col[0] + (-1)*ego_target_disX + (-3)*ego_target_disY + (0.01)*ego_dis + ego_extra
        npc_reward = (-5)*npc_col[0] + (-1)*npc_target_disX + (-3)*npc_target_disY + (0.01)*npc_dis + npc_extra
