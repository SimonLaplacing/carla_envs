...
class Create_Envs(object):
    ...
    def Create_actors(self): 
        ...
        self.npc_transform = Transform(Location(x=160.341522, y=-371.640472, z=0.281942), 
                    Rotation(pitch=0.000000, yaw=0.500910, roll=0.000000))
        for i in range(1):
            self.npc_transform.location += carla.Location(x=-15,y=-3.5)
        ...
        for i in range(28):
            if i == 0:
                obsta_bp = self.blueprint_library.find(id='vehicle.mercedes-benz.coupe')
                obstacle_transform.location += carla.Location(x=40,y=0.3)
                obstacle = self.world.try_spawn_actor(obsta_bp, obstacle_transform)
    ...
    def generate_path(self,vehicle,pathplanner,f_idx):
        ...
        fpath, fplist, best_path_idx = pathplanner.run_step(state, f_idx, None, self.obstacle_list, target_speed=30/3.6)
    ...
    def set_vehicle_control(self,ego_action,npc_action,ego_step,npc_step):
        if not self.args.direct_control:
            ...
            ego_route = self.ego_path[ego_step]
            npc_route = self.npc_path[npc_step]
    ...
    def get_vehicle_step(self, ego_step, npc_step, step):
        ...
        if self.args.Start_Path:
            if step ==0:
                ego_pathplanner = self.update_route(self.ego_route)
                npc_pathplanner = self.update_route(self.npc_route)

            if self.ego_f_idx >= self.ego_wps_to_go:
                ego_path, self.ego_wps_to_go, ego_fp = self.get_path(ego,ego_pathplanner,self.ego_f_idx)
                self.ego_f_idx = 1
                
            if self.npc_f_idx >= self.npc_wps_to_go:
                npc_path, self.npc_wps_to_go, npc_fp = self.get_path(npc,npc_pathplanner,self.npc_f_idx)
                self.npc_f_idx = 1
        ...
        if self.args.Start_Path:
            self.ego_f_idx = misc.closest_wp_idx(ego_location, ego_path, self.ego_f_idx)
            self.npc_f_idx = misc.closest_wp_idx(npc_location, npc_path, self.npc_f_idx)
        ...
        if self.args.Start_Path:
            ego_f_path,_,_ = misc.inertial_to_frenet(ego_route,ego_path[self.ego_f_idx][0],ego_path[self.ego_f_idx][1],ego_velocity.x,ego_velocity.y,ego_yaw)
            npc_f_path,_,_ = misc.inertial_to_frenet(npc_route,npc_path[self.npc_f_idx][0],npc_path[self.ego_f_idx][1],npc_velocity.x,npc_velocity.y,npc_yaw)
        ...
        #simple reward
        ego_reward = (-1)*ego_col[0] + (-0.6)*timeout + 0.6*ego_bonus
        npc_reward = (-1)*npc_col[0] + (-0.6)*timeout + 0.6*npc_bonus

        #reward shaping
        # ego_reward = ((-100)*ego_col[0] + (0.001)*(ego_dis + ego_ob) 
        # + (-1)*(ego_target_disX/5)**2 + (-10)*(ego_target_disY/10)**2 + (-20)*np.abs(np.sin(ego_yaw/2)) 
        # + (-0.5)*(ego_next_disX/10)**2 + (-5)*(ego_next_disY/10)**2 + (-10)*np.abs(np.sin(ego_next_yaw/2))
        # + 50*ego_bonus - 50*timeout
        # - 0.25*abs(ego_acc[1]))
        # npc_reward = ((-100)*npc_col[0] + (0.001)*(npc_dis + npc_ob)
        # + (-1)*(npc_target_disX/5)**2 + (-10)*(npc_target_disY/10)**2 + (-20)*np.abs(np.sin(npc_yaw/2))
        # + (-0.5)*(npc_next_disX/10)**2 + (-5)*(npc_next_disY/10)**2 + (-10)*np.abs(np.sin(npc_next_yaw/2)) 
        # + 50*npc_bonus - 50*timeout
        # - 0.25*abs(npc_acc[1]))
