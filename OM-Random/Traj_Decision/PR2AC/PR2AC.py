...
class Actor_Critic_RNN(nn.Module):
    ...
    def actor(self, s, p):
        ...
        s_ = self.activate_func(self.actor_fc11(s))
    ...
    def OM(self, s, p, a):
        s_ = torch.cat([s,a],-1)
        s_ = self.activate_func(self.OM_fc11(s_))
...
class PPO_RNN:
    ...
    def get_value(self, s, p, om_a):
        ...
        with torch.no_grad():
            ...
            if om_a is not None:
                ...
                om_a = torch.as_tensor(om_a, dtype=torch.float).unsqueeze(0)
                om_a = om_a.to(device)
                values = self.ac.critic(s, p, om_a)
            else:
                ...
                for _ in range(self.args.N//2):
                    ...
                    for _ in range(self.args.M//2):
                        om_a = om_dist_now.sample()  # Sample the action according to the probability distribution
                        om_a = torch.clamp(om_a, -1, 1)  # [-max,max]
    ...
    def train(self, total_steps, replay_buffer1, replay_buffer2=None):
        ...
        if replay_buffer2==None:
            max_episode_len = replay_buffer1.max_episode_len
            batch = replay_buffer1.get_training_data(max_episode_len,replay_buffer2)  # Get training data
            batch_size = self.batch_size
            mini_batch_size = self.mini_batch_size
        else:
            max_episode_len = max(replay_buffer1.max_episode_len,replay_buffer2.max_episode_len)
            batch = replay_buffer1.get_training_data(max_episode_len,replay_buffer2)  # Get training data
            batch2 = replay_buffer2.get_training_data(max_episode_len,replay_buffer1)  # Get training data
            for key in batch:
        ...
        try:
            for _ in range(self.K_epochs):
                for index in BatchSampler(SubsetRandomSampler(range(batch_size)), mini_batch_size, False):
                    ...
                    om_a_now,_,_ = self.ac.OM(batch['s'][index].to(device),batch['p'][index].to(device),a_mean)
