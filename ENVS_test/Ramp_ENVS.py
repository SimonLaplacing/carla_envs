...
class Create_Envs(object):
    ...
    def Create_actors(self,world, blueprint_library): 
        ...
        # 坐标建立
        ego_transform = Transform(Location(x=102, y=0, z=11), 
        ...
        # npc设置--------------------------------------------------------------------
        npc_transform = Transform(Location(x=110, y=6.5, z=11), 
        ...
        # npc设置--------------------------------------------------------------------
        npc_transform = Transform(Location(x=130, y=6.5, z=11), 
                    Rotation(pitch=0.000000, yaw=180.500910, roll=0.000000))
        for i in range(1):
            # npc_transform.location += carla.Location(x=-15,y=-3.5)
            npc_bp = blueprint_library.find(id='vehicle.lincoln.mkz2017')
            # print(npc_bp.get_attribute('color').recommended_values)
            npc_bp.set_attribute('color', '229,128,0')
            npc = world.try_spawn_actor(npc_bp, npc_transform)
            if npc is None:
                print('%s npc created failed' % i)
            else:
        ...
        # npc设置--------------------------------------------------------------------
        npc_transform = Transform(Location(x=80, y=6.5, z=11), 
                    Rotation(pitch=0.000000, yaw=180.500910, roll=0.000000))
        for i in range(1):
            # npc_transform.location += carla.Location(x=-15,y=-3.5)
            npc_bp = blueprint_library.find(id='vehicle.lincoln.mkz2017')
            # print(npc_bp.get_attribute('color').recommended_values)
            npc_bp.set_attribute('color', '0,128,213')
            npc = world.try_spawn_actor(npc_bp, npc_transform)
            if npc is None:
                print('%s npc created failed' % i)
            else:
        ...
        # 障碍物设置------------------------------------------------------------------
        obstacle_transform = Transform(Location(x=0, y=6.9, z=11), 
                    Rotation(pitch=0.000000, yaw=180.500910, roll=0.000000))
        for i in range(52):
            if i == 0:
                obsta_bp = blueprint_library.find(id='vehicle.mercedes-benz.coupe')
                # obstacle_transform.location += carla.Location(x=-95)
                # obstacle = world.try_spawn_actor(obsta_bp, obstacle_transform)
                obstacle_transform.location += carla.Location(x=0,y=1.3)
                # if obstacle is None:
