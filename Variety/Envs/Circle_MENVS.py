...
class Create_Envs(object):
    ...
    def Create_actors(self): 
        ...
        if self.agent_num>=2:
            ...
            if self.agent_num>=3:
                ...
                if self.agent_num>=4:
                    ...
                    ego_bp = self.blueprint_library.find(id='vehicle.lincoln.mkz2017')
                    # 坐标建立
                    self.ego_transform = Transform(Location(x=19.2+deltaX[3], y=0+deltaY[3], z=0.1), 
                                Rotation(pitch=0,yaw=270+deltaYaw[3], roll=-0.000000))
        ...
        obstacle_transform5 =Transform(Location(x=6, y=25, z=0), 
                    Rotation(pitch=0, yaw=135, roll=0.000000))
        for i in range(10):
            obstacle5 = self.world.try_spawn_actor(obsta_bp, obstacle_transform5)
            obstacle_transform5.location += carla.Location(x=1.7,y=-1.5)
            self.obstacle_list.append(obstacle5)
        #障碍物8
        obstacle_transform6 =Transform(Location(x=6, y=-24, z=0), 
                    Rotation(pitch=0, yaw=45, roll=0.000000))
        for i in range(10):
            obstacle6 = self.world.try_spawn_actor(obsta_bp, obstacle_transform6)
            obstacle_transform6.location += carla.Location(x=1.7,y=1.5)
            self.obstacle_list.append(obstacle6)
        for i in range(1):
    ...
    def get_route(self):
        ...
        delta = [carla.Location(x=60,y=-23.5),carla.Location(x=75,y=0),carla.Location(x=75,y=0),carla.Location(x=75,y=0)]
        for i in range(self.agent_num):
