...
import copy
import math
import Path_Decision.cubic_spline_planner as cubic_spline_planner
...
class FrenetPlanner:
    def __init__(self,args):
        ...
        self.args = args
        if float(self.args.fixed_delta_seconds) > 0:
            self.dt = float(self.args.fixed_delta_seconds)
        else:
            self.dt = 0.05
        ...
        self.ROBOT_RADIUS = 0  # robot radius [m]
    ...
    def update_global_route(self, global_route):
        ...
        for p in global_route:
            wx.append(p.location.x)
            wy.append(p.location.y)
            wz.append(p.location.z)
        # print('ppppp:', wx)
    ...
    def estimate_frenet_state(self, ego_state, idx):
        ...
        # print('aaaaaaa: ',update_s(estimated_s))
    ...
    def find_surrounding_actors(self, s, d, actors_batch=None):
            ...
            for actor in actors_batch:
                ...
                if lane_ == lane - 1:
                    ...
                    if s_diff >= 0 and s_diff < ld_min:
                        ...
                        sur_actos['Left Down']['Speed'] = 0
                        sur_actos['Left Down']['Target Speed'] = 0
                    # up
                    elif s_diff < 0 and abs(s_diff) < lu_min:
                        lu_min = abs(s_diff)
                        sur_actos['Left Up']['Exist'] = True
                        sur_actos['Left Up']['Frenet State'] = [s_, d_]
                        sur_actos['Left Up']['Speed'] = 0
                        sur_actos['Left Up']['Target Speed'] = 0
                ...
                # current lane
                    ...
                    if s_diff >= 0 and s_diff < cd_min:
                        ...
                        sur_actos['Center Down']['Speed'] = 0
                        sur_actos['Center Down']['Target Speed'] = 0
                    # up
                    elif s_diff < 0 and s_diff < cu_min:
                        cd_min = s_diff
                        sur_actos['Center Up']['Exist'] = True
                        sur_actos['Center Up']['Frenet State'] = [s_, d_]
                        sur_actos['Center Up']['Speed'] = 0
                        sur_actos['Center Up']['Target Speed'] = 0
                ...
                # right
                    ...
                    if s_diff >= 0 and s_diff < rd_min:
                        ...
                        sur_actos['Right Down']['Speed'] = 0
                        sur_actos['Right Down']['Target Speed'] = 0
                    # up
                    elif s_diff < 0 and abs(s_diff) < ru_min:
                        ru_min = abs(s_diff)
                        sur_actos['Right Up']['Exist'] = True
                        sur_actos['Right Up']['Frenet State'] = [s_, d_]
                        sur_actos['Right Up']['Speed'] = 0
                        sur_actos['Right Up']['Target Speed'] = 0
        ...
        for fp in fplist:
            ...
            for i in range(1, len(fp.t) - 1):
                ...
                k = np.sqrt((a + (b + c)) * (c - (a - b)) * (c + (a - b)) * (
                        a + (b - c))) / 4  if (a + (b + c)) * (c - (a - b)) * (c + (a - b)) * (a + (b - c)) > 0 else 0 # Heron's formula for triangle's surface
    ...
    def check_paths(self, fplist, ob_list):
        """
                    Jm = -1 * (ae_ - ae + p*(an_ - an) + q*(ao_ - ao))
    ...
    def frenet_optimal_planning(self, f_state, other_actors, ob_list, target_speed=30 / 3.6):
        """
        ...
        # print('fp1:  ',len(fplist))
        fplist = self.calc_global_paths(fplist) # 路径全局坐标转换
        # print('fp2:  ',len(fplist))
        fplist = self.calc_curvature_paths(fplist) # 路径全局曲率坐标转换
        # print('fp3:  ',len(fplist))
        fplist = self.check_paths(fplist, ob_list) # 碰撞路径筛选
        # print('fp4:  ',len(fplist))
    ...
    def reset(self, s=50, d=0, df_n=0, Tf=4, Vf_n=0, optimal_path=True):
        # module_world reset should be executed beforehand to update the initial s and d values
        init_s = 50  # ego initial s location
        ...
        init_d = 0 * self.LANE_WIDTH  # -1 and 3 because global route is defined on the second lane from left
        f_state = [init_s, 0, 0, init_d, 0, 0]

        if optimal_path:
            best_path_idx, fplist = self.frenet_optimal_planning(f_state,None,[])
    ...
    def run_step(self, ego_state, idx, other_actors, ob_list, target_speed=30 / 3.6):
        """
        ...
        best_path_idx, fplist = self.frenet_optimal_planning(f_state, other_actors, ob_list, target_speed=target_speed)
        print('best:  ',best_path_idx,len(fplist))
        if len(fplist) == 0:
            return 0,0,0
        else:
            self.path = fplist[best_path_idx]
