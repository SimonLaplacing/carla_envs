...
def _kinematics_from_tokens(position_list, yaw_list, velocity_list) -> KinematicsData:
    """
    ...
    x, y = position_list[-1,0], position_list[-1,1]
    yaw = yaw_list[-1]
    velocity = velocity_list[-1]
    acceleration = (velocity_list[-1] - velocity_list[-2]) / 0.1
    yaw_rate = (yaw_list[-1] - yaw_list[-2]) / 0.1
...
class Baseline(abc.ABC):
    ...
    def __init__(self, sec_from_now: float):
        ...
        self.sampled_at = 10  # 2 Hz between annotations.
...
class ConstantVelocityHeading(Baseline):
    ...
    def __call__(self, data) -> np.ndarray:
        ...
        """
        kinematics = _kinematics_from_tokens(data['history_position'], data['history_position'], data['history_yaw'])
        ...
        return np.expand_dims(cv_heading, 0), np.array([1])
...
class PhysicsOracle(Baseline):
    ...
    def __call__(self, data):
        ...
        :param token: string of format {instance_token}_{sample_token}.
        """
        kinematics = _kinematics_from_tokens(data['history_positions'], data['history_yaws'], data['history_velocities'])
        ground_truth = data['target_positions']
        ...
        # Need the prediction to have 2d.
        return np.expand_dims(oracle, 0), np.array([1])
