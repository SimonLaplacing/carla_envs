...
def _average_displacement_error(
    ...
    if mode == "oracle":
        ...
        error1 = np.mean(error[np.argsort(confidences)[-1:]], axis=0)  # average over hypotheses
        error2 = np.mean(error[np.argsort(confidences)[-5:]], axis=0)  # average over hypotheses
        error3 = np.mean(error[np.argsort(confidences)[-10:]], axis=0)  # average over hypotheses
